...
{
    public partial class RunningAwayFinished : MetroForm
    {
        Timer timer = new Timer();
        int count_down = 34;

        public RunningAwayFinished()
        {
            InitializeComponent();
            string path = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), @"..\..\Story\Story 1\Sounds\running away finished.wav");
